...
filter(medL,medLifeExp<40)
filter(medL,medLifeExp>60)
cc = c("Angola","Guinea-Bissau","Sierra Leone",
       "Mauritius","Reunion","Tunisia",
       "Mexico") # Mexico for comparison

gapminderCC = filter(gapminder,country %in% cc)
ggplot(gapminderCC, aes(x=year,y=lifeExp,color=country)) +
 geom_point() +
 geom_smooth(method = "lm", se=FALSE)
```