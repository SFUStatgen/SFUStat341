...
h + geom_violin(aes(x=continent,y=lifeExp)) +
   stat_summary(aes(x=continent,y=lifeExp),fun.data="mean_cl_normal",color="red")
```
...
- Basic tools for annotating are
    ...
    passed to the function, rather than mapped from data.
...
p <- ggplot(gapminder,aes(x=gdpPercap,y=lifeExp,color=continent)) + geom_point(alpha=0.5) + coord_trans(x="log10")
p
```
