...
`for()` and `while()` loops.
    - These are "imperative programming" tools, that 
    make the iteration explicit.
...
summarize(gm_byContinent,min(lifeExp),median(lifeExp), IQR(lifeExp),mean(lifeExp),sd(lifeExp),max(lifeExp))
options(oldops)
```
...
applied directly to the geom it is to apply t
...
```{r, fig.width=4, fig.height=2}
ggplot(gapminder,aes(x=gdpPercap,y=lifeExp,color=continent)) + geom_point(alpha=0.5) + coord_trans(x="log10")
```
